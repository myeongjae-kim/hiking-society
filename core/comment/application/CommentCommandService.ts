...
export class CommentCommandService implements CommentCommandUseCase {
  ...
  async create(input: Parameters<CommentCommandUseCase['create']>[0]) {
    const article = await this.commentCommandPort.findActiveArticleById(input.articleId);

    if (!article) {
      throw applicationError.notFound('댓글을 작성할 글을 찾을 수 없습니다.');
    }

    const parentCommentId = 'parentCommentId' in input ? input.parentCommentId : null;
  ...
  async update(input: Parameters<CommentCommandUseCase['update']>[0]) {
    ...
    const comment = await this.commentCommandPort.findCommentById(input.commentId);

    if (!comment || comment.authorUserId !== input.userId) {
      throw applicationError.notFound('댓글을 수정할 권한이 없거나 댓글을 찾을 수 없습니다.');
    }
  ...
  async delete(input: Parameters<CommentCommandUseCase['delete']>[0]) {
    ...
    if (!comment || comment.authorUserId !== input.userId) {
      throw applicationError.notFound('댓글을 삭제할 권한이 없거나 댓글을 찾을 수 없습니다.');
    }
