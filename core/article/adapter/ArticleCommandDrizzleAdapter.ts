...
import type { ArticleId, ArticleMediaMetadataSummary } from '@/core/article/domain';
import type {
...
export class ArticleCommandDrizzleAdapter implements ArticleCommandPort {
  ...
  async delete(input: Parameters<ArticleCommandPort['delete']>[0]) {
    const [updated] = await db
      ...
      .where(and(eq(articleTable.id, toNumericId(input.articleId)), isNull(articleTable.deletedAt)))
      .returning({ id: articleTable.id });
  ...
  async hasActiveHiking(hikingId: HikingId) {
    const [hiking] = await db
      ...
      .from(hikingTable)
      .where(and(eq(hikingTable.id, toNumericId(hikingId)), isNull(hikingTable.deletedAt)))
      .limit(1);
  ...
  async listActiveNotificationRecipientIds(
    input: Parameters<ArticleCommandPort['listActiveNotificationRecipientIds']>[0],
  ) {
    const recipients = await db
      .select({ id: userTable.id })
      .from(userTable)
      .where(and(isNull(userTable.deletedAt), ne(userTable.id, input.excludeUserId)));
