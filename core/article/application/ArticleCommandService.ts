...
export class ArticleCommandService implements ArticleCommandUseCase {
  ...
  async create(input: Parameters<ArticleCommandUseCase['create']>[0]) {
    if (input.media.length === 0) {
      throw applicationError.badRequest('글은 사진이나 동영상 없이 저장할 수 없습니다.');
    }

    const hasActiveHiking = await this.articleCommandPort.hasActiveHiking(input.hikingId);
    ...
    const recipientUserIds = await this.articleCommandPort.listActiveNotificationRecipientIds({
  ...
  async update(input: Parameters<ArticleCommandUseCase['update']>[0]) {
    ...
    const article = await this.articleCommandPort.findActiveArticleById(input.articleId);

    if (!article || article.authorUserId !== input.userId) {
      throw applicationError.notFound('글을 수정할 권한이 없거나 글을 찾을 수 없습니다.');
    }
  ...
  async delete(input: Parameters<ArticleCommandUseCase['delete']>[0]) {
    ...
    if (!article || article.authorUserId !== input.userId) {
      throw applicationError.notFound('글을 삭제할 권한이 없거나 글을 찾을 수 없습니다.');
    }
