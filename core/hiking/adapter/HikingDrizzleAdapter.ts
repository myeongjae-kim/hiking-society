...
import { articleTable, hikingTable } from '@/lib/db/schema';
  ...
  async findActiveHikingById(hikingId: HikingId) {
    ...
    const [hikingRows, articleCountRows] = await Promise.all([
      db
        ...
        .from(hikingTable)
        .where(and(eq(hikingTable.id, numericHikingId), isNull(hikingTable.deletedAt)))
        .limit(1),
      db
        .select({ articleCount: sql<number>`count(*)::int` })
        .from(articleTable)
        .where(and(eq(articleTable.hikingId, numericHikingId), isNull(articleTable.deletedAt))),
    ]);
