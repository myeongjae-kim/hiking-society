...
export class ProfileDrizzleAdapter implements ProfileCommandPort, ProfileQueryPort {
  async existsActiveUserByEmailExceptUserId(
    input: Parameters<ProfileQueryPort['existsActiveUserByEmailExceptUserId']>[0],
  ) {
    const [row] = await db
  ...
  async updateActiveProfile(input: Parameters<ProfileCommandPort['updateActiveProfile']>[0]) {
    await db
      .update(userTable)
      .set({
