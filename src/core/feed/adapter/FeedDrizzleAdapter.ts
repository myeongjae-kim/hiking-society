...
function toArticleMedia(
	media: readonly ArticleMedia[],
): ArticleMediaItems | null {
	if (media.length === 0) {
		return null;
	}
...
export class FeedDrizzleAdapter implements FeedQueryPort {
	...
	async listHikings() {
		return this.transactionRunner.read(async (tx) => {
			...
			return {
				articleCount: hikingArticleCounts.reduce(
					(total, row) => total + row.articleCount,
					0,
				),
	...
	async listHikingArticles(
		...
		return this.transactionRunner.read(async (tx) => {
			...
			const mediaRows = await tx
				...
				.innerJoin(
					articleTable,
					eq(articleTable.id, articleMediaTable.articleId),
				)
				.where(
					and(
						eq(articleTable.hikingId, hikingId),
			...
			const articleLikeRows = await tx
				.select({
