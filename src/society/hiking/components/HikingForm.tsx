...
import { FieldLabel } from "#/society/shared/components/FieldLabel";
import {
	boxBorderClassName,
	fieldClassName,
...
function shiftTimeValue(time: string, offsetMinutes: number) {
	...
	const shiftedMinutes =
		(hour * 60 + minute + offsetMinutes + minutesInDay) % minutesInDay;
...
function isExifPhotoFile(file: File) {
	return (
		exifPhotoTypes.has(file.type.toLowerCase()) ||
		exifPhotoFileNamePattern.test(file.name)
	);
}
...
export function HikingForm({
	...
	const readMetadataFromFiles = async (files: File[]) => {
		...
		if (!file) {
			setMetadataStatus(
				"EXIF를 읽을 수 있는 JPEG/TIFF/HEIC/HEIF 사진만 지원합니다.",
			);
			return;
		}
		...
		try {
			...
			if (!hasGps && !hasAltitude && !metadata.takenAt) {
				setMetadataStatus(
					"이 사진에서 EXIF 좌표, 고도, 촬영시각을 찾지 못했습니다.",
				);
				return;
			}
			...
			setValues((currentValues) => ({
				...
				...(hasGps
					? {
							...
							longitude:
								metadata.longitude?.toFixed(6) ?? currentValues.longitude,
						}
					: {}),
				...(metadata.takenAt
					? {
			...
			if (metadata.takenAt) {
				appliedLabels.push(
					`촬영시각 ${metadata.takenAt.date} ${metadata.takenAt.time}`,
				);
			} else {
	...
	const handleMetadataDropAreaDrop = async (
			...
			<div className="grid gap-3 sm:grid-cols-2">
				<FieldLabel label="산 이름">
					<input
						...
						onChange={(event) =>
							updateValue("mountainName", event.currentTarget.value)
						}
						required
				...
				<FieldLabel label="산행 날짜">
					<input
						className={fieldClassName}
						onChange={(event) =>
							updateValue("hikingDate", event.currentTarget.value)
						}
						required
						type="date"
				...
				<FieldLabel label="타임존">
					<input
						className={fieldClassName}
						onChange={(event) =>
							updateValue("timezone", event.currentTarget.value)
						}
						required
				...
				<FieldLabel label="참석자 (쉼표로 구분)">
					<input
						className={fieldClassName}
						onChange={(event) =>
							updateValue("participantsCsv", event.currentTarget.value)
						}
						required
				...
				<FieldLabel label="출발 시각">
					<input
						className={fieldClassName}
						onChange={(event) =>
							updateValue("startedTime", event.currentTarget.value)
						}
						required
						type="time"
				...
				<FieldLabel label="하산 완료 시각">
					<input
						className={fieldClassName}
						onChange={(event) =>
							updateValue("completedTime", event.currentTarget.value)
						}
						required
						type="time"
				...
				<FieldLabel label="위도">
					<input
						...
						inputMode="decimal"
						onChange={(event) =>
							updateValue("latitude", event.currentTarget.value)
						}
						required
				...
				<FieldLabel label="경도">
					<input
						...
						inputMode="decimal"
						onChange={(event) =>
							updateValue("longitude", event.currentTarget.value)
						}
						required
				...
				<FieldLabel label="고도 (m, 생략 가능)">
					<input
						...
						inputMode="decimal"
						onChange={(event) =>
							updateValue("altitude", event.currentTarget.value)
						}
				...
				<FieldLabel label="식당 주소 (생략 가능)">
					<input
						className={fieldClassName}
						onChange={(event) =>
							updateValue("restaurantAddress", event.currentTarget.value)
						}
