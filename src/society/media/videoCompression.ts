...
let ffmpegInstancePromise: Promise<import("@ffmpeg/ffmpeg").FFmpeg> | null =
	null;
...
function readVideoMetadata(file: File) {
	return new Promise<{ durationMs: number; height: number; width: number }>(
		(resolve, reject) => {
...
function toUint8Array(
	data: Awaited<ReturnType<import("@ffmpeg/ffmpeg").FFmpeg["readFile"]>>,
) {
...
export async function createCompressedMp4File(
	...
	const sourceThumbnailPromise = createThumbnail(file, file.name).catch(
		() => null,
	);
	...
	try {
		...
		const thumbnailFile =
			sourceThumbnailFile ?? (await createThumbnail(outputFile, file.name));
