...
export function useInlineCarouselGesture({
	...
	const handleInlinePointerMove = useCallback(
		(event: PointerEvent<HTMLButtonElement>) => {
			...
			if (
				!inlineSwipeGesture ||
				inlineSwipeGesture.pointerId !== event.pointerId
			) {
				return;
			}
			...
			const axis =
				inlineSwipeGesture.axis ?? getInlineSwipeAxis(deltaX, deltaY);
			...
			if (axis === "horizontal") {
				...
				setInlineSwipeTrack(
					createInlineSwipeTrack(
						clamp(deltaX, -maxSwipeOffsetX, maxSwipeOffsetX),
						false,
					),
				);
				return;
	...
	const handleInlineTouchMove = useCallback(
		(event: TouchEvent<HTMLButtonElement>) => {
			...
			const touch = getTouchByIdentifier(
				event.touches,
				inlineSwipeGesture.pointerId,
			);
