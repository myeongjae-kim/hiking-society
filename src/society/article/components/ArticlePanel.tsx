...
import { MediaViewer } from "#/society/article/components/MediaViewer";
import { CommentForm } from "#/society/comment/components/CommentForm";
import { CommentLine } from "#/society/comment/components/CommentLine";
import {
	getThreadedComments,
	getVisibleCommentCount,
} from "#/society/comment/components/commentUtils";
import { ActionButton } from "#/society/shared/components/ActionButton";
import type { AuthorAvatarRenderer } from "#/society/shared/components/AuthorBadge";
import { AuthorBadge } from "#/society/shared/components/AuthorBadge";
import { Command } from "#/society/shared/components/Command";
import { InlineMeta } from "#/society/shared/components/InlineMeta";
import {
	boxBorderClassName,
...
export function ArticlePanel({
	...
	const visibleCommentIds = visibleCommentThreads.flatMap(
	...
	): AuthorAvatarRenderer | undefined =>
		profileImageUrl
			? (avatar) => (
					<MediaViewer
	...
	return (
		...
		>
			<header className="grid gap-2">
				...
				<div className="font-mono">
					<AuthorBadge
						...
						renderAvatar={renderProfileAvatar(
							article.authorName,
				...
				<InlineMeta
					items={getArticleMeta(
						article,
						getVisibleCommentCount(comments),
						articleDetailHref,
					)}
				/>
			</header>
				...
				<div>
					...
					>
						<span className="inline-flex items-center gap-2">
							<span
								className={
									article.likedByCurrentUser ? "text-[var(--red)]" : undefined
								}
							>
			...
			>
				<Command>
					comments.list --count={getVisibleCommentCount(comments)}
				</Command>
						...
						{replyingCommentId === comment.id ? (
							<div className="ml-4">
								<CommentForm
									...
									onSubmit={(body) =>
										onCreateComment(article.id, body, comment.id)
									}
									prompt="reply.new>"
