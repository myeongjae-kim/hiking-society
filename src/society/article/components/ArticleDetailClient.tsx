...
import { getAuthorName } from "#/society/feed/utils/feed-crud-utils";
import { getHikingDisplay } from "#/society/hiking/components/hikingFormUtils";
import Link from "#/society/shared/components/AppLink";
import { Command } from "#/society/shared/components/Command";
import {
	ConfirmDialog,
	type ConfirmState,
} from "#/society/shared/components/ConfirmDialog";
...
const getCommentCreateSingleFlightKey = (
	articleId: ArticleId,
	parentCommentId: CommentId | null,
) => `comment-create-${articleId}-${parentCommentId ?? "root"}`;

const getCommentUpdateSingleFlightKey = (commentId: CommentId) =>
...
export function ArticleDetailClient({
	...
	const articleUpdateSubmitting =
		isRunning(articleUpdateSingleFlightKey) ||
		...
		runMutation(
			{
				...
				singleFlightKey: getCommentCreateSingleFlightKey(
					articleId,
					parentCommentId,
				),
			},
			...
			onConfirm: () => {
				runMutation(
					...
					async () => {
						...
						setConfirmState(null);
						invalidateQueryKeys([
							apiQueryKeys.articleComments(comment.articleId),
						]);
						router.refresh();
					},
				);
		...
		runMutation(
			...
			async () => {
				...
				setArticleLikeOverride({
					...
					likeCount: Math.max(
						0,
						displayedArticle.likeCount +
							(displayedArticle.likedByCurrentUser ? -1 : 1),
					),
				});
				invalidateQueryKeys([
		...
		runMutation(
			...
			async () => {
				...
				setCommentLikeOverrides((currentOverrides) => {
					const currentComment = displayedComments.find(
		...
		<main className="min-h-svh bg-[length:2rem_2rem] bg-[linear-gradient(var(--surface0)_1px,transparent_1px),linear-gradient(90deg,var(--surface0)_1px,transparent_1px),var(--background0)] text-[var(--foreground0)]">
			...
			<div className="mx-auto grid w-[min(100%,62rem)] gap-4 px-1.5 py-4 sm:px-4 lg:p-5">
				...
				<ArticlePanel
					...
					articleLikePending={
						pendingLikeByKey[`article-${displayedArticle.id}`] === true
					}
			...
			<LoadingOverlay
				...
				open={isPending && loadingLabel !== null}
			/>
		</main>
	);
}
