	...
	return (
		<main className="min-h-svh bg-[length:2rem_2rem] bg-[linear-gradient(var(--surface0)_1px,transparent_1px),linear-gradient(90deg,var(--surface0)_1px,transparent_1px),var(--background0)] text-[var(--foreground0)]">
			...
			<section className="mx-auto grid min-h-[calc(100svh-4rem)] w-[min(100%,64rem)] place-items-center p-4 lg:p-8">
				<div
					className={`!p-5 grid w-full gap-6 bg-[color-mix(in_srgb,var(--background0)_94%,var(--surface0))] ${boxBorderClassName} md:grid-cols-[minmax(0,1fr)_minmax(16rem,26rem)] md:items-center`}
					box-="round"
				>
					<div className="grid min-w-0 gap-4">
						<Command>feed.access --associate</Command>
						<h1 className="m-0 text-3xl text-[var(--blue)] leading-[1.15] sm:text-4xl">
							환영합니다! 준회원은 아직 피드를 조회할 수 없어요.
						</h1>
						<p className="m-0 text-[var(--subtext0)] text-sm leading-[1.6]">
							기존 멤버에게 연락해주세요. 가입 확인이 끝나면 이곳에서 산행
							기록과 글을 볼 수 있습니다.
						</p>
