...
export function NotificationPopover({
	...
	const markAllNotificationsReadMutation = $api.useMutation(
		"patch",
	...
	const [readNotificationIds, setReadNotificationIds] = useState<
		ReadonlySet<string>
	>(() => new Set());
	const [allReadAt, setAllReadAt] = useState<
		NotificationSummary["readAt"] | null
	>(null);
	...
	const hasUnreadNotifications = allReadAt
		? false
		: lastPage.hasUnreadNotifications;
									...
									return (
										<button
											className={`${notificationItemClassName} ${
												unread
													? "shadow-[inset_0.25rem_0_0_var(--red)]"
													: "opacity-75"
											}`}
											disabled={
												isPending || markNotificationReadMutation.isPending
											}
											key={notification.id}
