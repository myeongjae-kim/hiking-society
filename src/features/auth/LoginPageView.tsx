...
const heroTitleFrameSteps: HeroTitleFrame[] = [
	...
	{ before: "대학생등산동아리", duration: 2000 },
	{
		before: "대학생등산동아",
		after: "리",
		duration: heroTitleEditFrameDuration,
	},
	{
		before: "대학생등산동",
		after: "아리",
	...
	},
	{
		before: "대학생",
		after: "등산동아리",
		duration: heroTitleEditFrameDuration * 3,
	},
	{
		before: "대학생(",
		after: "등산동아리",
	...
	},
	{
		before: "대학생(?)",
		after: "등산동아리",
		duration: heroTitleEditFrameDuration * 10,
	},
	{ before: finalHeroTitle, final: true },
];

const heroTitleFrames = heroTitleFrameSteps.reduce<TimedHeroTitleFrame[]>(
	(frames, frame) => {
		const previousFrame = frames.at(-1);
		const delay = previousFrame
			? previousFrame.delay + previousFrame.duration
			: 0;
...
function getHeroTitleBreakIndex(frame: HeroTitleFrame) {
	...
	if (
		fullText.startsWith(`${studentBreak}등`) ||
		(frame.before === studentBreak && after)
	) {
...
export default function Home({ redirectTo = "/feed" }: HomeProps) {
	const finalHeroTitleLines = getHeroTitleLines({
		before: finalHeroTitle,
		final: true,
	});
