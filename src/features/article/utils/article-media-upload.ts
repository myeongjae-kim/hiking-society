...
type ArticleMediaUploadTargets = {
	targets: {
		objectKey: string;
		thumbnail?: {
			objectKey: string;
...
export type ArticleMediaUploadTargetCreator = (
	body: ArticleMediaUploadTargetInput[],
) => Promise<ArticleMediaUploadTargets>;

export type ArticleMediaUploadCleanup = (
...
export function createArticleMediaUploader({
	createUploadTargets,
	deleteUploads,
}: {
	createUploadTargets: ArticleMediaUploadTargetCreator;
	deleteUploads: ArticleMediaUploadCleanup;
}) {
	const deleteUploadedArticleMedia = async (objectKeys: readonly string[]) => {
		if (objectKeys.length === 0) {
			return;
		}

		await deleteUploads(objectKeys);
	};
	...
	const uploadArticleMedia = async (
		...
		const targetInput: ArticleMediaUploadTargetInput[] = newMedia.map(
			(media) => ({
		...
		const targetResult = await createUploadTargets(targetInput);
		...
		try {
			await runWithConcurrency(newMedia, 4, async (media, index) => {
				...
				uploadedCount += 1;
				onLoadingLabelChange(
					`S3 업로드 중 ${uploadedCount}/${newMedia.length}`,
				);
			});
		} catch (error) {
	...
	return { deleteUploadedArticleMedia, uploadArticleMedia };
}
