...
function getWebpFileName(fileName: string) {
	...
	const baseName =
		extensionStartIndex > 0
			? fileName.slice(0, extensionStartIndex)
			: fileName || "image";
...
export function isHeicImageFile(file: File) {
	return (
		...
		heicFileNamePattern.test(file.name)
	);
}
...
async function encodeImageDataToWebpBlob(
	imageData: ImageData,
	quality: number,
) {
...
function normalizeExifOrientation(exif: Bytes): Bytes {
	const tiffOffset = startsWithBytes(exif, 0, jpegExifHeader)
		? jpegExifHeader.length
		: 0;
	...
	const readUint32 = (offset: number) =>
		isLittleEndian
			? readUint32LittleEndian(exif, offset)
			: readUint32BigEndian(exif, offset);
...
function normalizeXmpOrientation(xmp: Bytes): Bytes {
	...
	const normalized = text
		.replace(/tiff:Orientation\s*=\s*(["'])[1-8]\1/g, (match) =>
			match.replace(/[1-8]/, "1"),
		)
		.replace(
...
function extractJpegXmpPayload(payload: Bytes) {
	return startsWithBytes(payload, 0, jpegXmpHeader)
		? payload.slice(jpegXmpHeader.length)
		: null;
}
...
async function extractOriginalMetadataChunks(
	file: File,
): Promise<WebpMetadataChunks> {
		...
		const extractedChunks = await Promise.all(
			blocks.map(async (block): Promise<WebpMetadataChunks> => {
				...
				if (block.type === "icc") {
					...
					const icc = isPng
						? payload
							? await extractPngIccPayload(payload)
							: null
						: payload;
...
async function attachMetadataToWebpBlob(
	...
	if (
		readAscii(sourceBytes, 0, 4) !== "RIFF" ||
		readAscii(sourceBytes, 8, 4) !== "WEBP"
	) {
		return webpBlob;
	}
	...
	if (readAscii(sourceBytes, 12, 4) === "VP8X") {
		...
		if (
			vp8xChunkSize !== webpVp8xChunkSize ||
			vp8xChunkEnd > sourceBytes.length
		) {
			return webpBlob;
		}
...
export async function compressPreparedImageSource(
	...
	const scale =
		longestSide > options.maxWidth ? options.maxWidth / longestSide : 1;
	...
	const blobWithMetadata = await attachMetadataToWebpBlob(
		blob,
		source.metadataChunks,
		{
			height: canvasHeight,
			width: canvasWidth,
		},
