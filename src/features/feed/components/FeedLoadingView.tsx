import { boxBorderClassName, gridStackClassName } from '#/features/shared/components/styles';

const skeletonBlockClassName = 'animate-pulse bg-[var(--surface1)]';

function SkeletonBlock({ className }: { className: string }) {
  return <span aria-hidden="true" className={`${skeletonBlockClassName} ${className}`} />;
}
    ...
    >
      ...
      <div className="grid gap-2">
        {Array.from({ length: 7 }, (_, index) => (
          <div className="grid grid-cols-[5rem_minmax(0,1fr)] items-center gap-2" key={index}>
      ...
      <div aria-live="polite" role="status">
        ...
        <header className="border-b border-[var(--overlay0)] bg-[color-mix(in_srgb,var(--background0)_92%,transparent)] px-4 py-3">
          <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1">
            ...
            <nav className="ml-auto flex flex-wrap items-center gap-2" aria-hidden="true">
