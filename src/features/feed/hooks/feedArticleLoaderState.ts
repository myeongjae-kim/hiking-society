...
export function createArticleHikingIdByArticleId(articles: readonly Article[]) {
  return new Map<ArticleId, HikingId>(articles.map((article) => [article.id, article.hikingId]));
}

export function createCommentArticleIdByCommentId(comments: readonly Comment[]) {
...
export function filterRecordByActiveHikingIds<T>(
  ...
  return Object.fromEntries(
    Object.entries(record).filter(([hikingId]) => activeHikingIds.has(hikingId as HikingId)),
  );
}
