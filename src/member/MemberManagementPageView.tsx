    ...
    <main className="min-h-svh bg-[linear-gradient(var(--surface0)_1px,transparent_1px),linear-gradient(90deg,var(--surface0)_1px,transparent_1px),var(--background0)] bg-[length:2rem_2rem] p-4 text-[var(--foreground0)] lg:p-8">
      ...
      >
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="m-0 font-mono text-sm text-[var(--mauve)]">$ members.list</p>
                  ...
                  <td>
                    <MemberRoleForm userId={member.id}>
                      ...
                      <select
                        ...
                        name="role"
                        disabled={
                          !roleOptions.some((role) => canChangeRole(actor.role, member.role, role))
                        }
                      >
                        {roleOptions.map((role) => (
                          <option
                            value={role}
                            key={role}
                            disabled={!canChangeRole(actor.role, member.role, role)}
                          >
                            {roleLabels[role]}
                          </option>
                        ))}
