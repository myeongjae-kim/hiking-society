...
const geocodingCache = new Map<string, { expiresAt: number; value: GeocodingSearchResponse }>();
...
function parseNominatimResult(result: NominatimSearchResult, index: number) {
  ...
  const label = typeof result.display_name === 'string' ? result.display_name : '';
...
controller.openapi(
  createRoute({
    ...
    responses: {
      200: {
        content: { 'application/json': { schema: geocodingSearchResponseSchema } },
  ...
  async (c) => {
    ...
    try {
      response = await fetch(url, {
        headers: {
          ...
          'User-Agent': 'hiking-society/0.1.0 (https://github.com/myeongjaekim/hiking-society)',
        },
      });
    } catch {
    ...
    const value = {
      results: body
        .map((item, index) => parseNominatimResult(item as NominatimSearchResult, index))
